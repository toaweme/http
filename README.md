...
`http` uses an issue-first workflow. Open an issue describing the change and wait for a maintainer to approve the approach (the `approved` label) before you open a pull request. PRs that don't reference an approved issue are flagged by a bot and usually closed, so the issue step saves you wasted work.
